...
async function startSoloVsAI(nPlayers){
  ...
  // v12.5.4: 단일 PUT 으로 봇 포함 완전한 방을 한 번에 생성
  //          (이전: createRoom + addBot×N 분리 — Firebase eventual consistency 로 봇 누락 발생)
  const rid = roomCode();
  const botNames = ['岐伯AI','華佗AI','扁鵲AI','張仲景AI','孫思邈AI'];
  const players = {};
  // 사용자 (seat 0, host)
  ...
  console.log('[方劑麻雀] 단일 PUT 시도', { rid, players: Object.keys(players).length });

  let ok = false;
  try{
    ok = await f.put(`${FB_NODE}/${rid}`, room);
  }catch(e){
    console.error('[方劑麻雀] AI 對局 PUT 예외', e);
    toast('서버 통신 오류','warn');
    return;
  }
  if(!ok){
    console.error('[方劑麻雀] AI 對局 PUT 실패 — Firebase 룰 또는 네트워크 확인', {
      path: `${FB_NODE}/${rid}`,
      hint: 'Firebase Console → Realtime Database → Rules 에서 "' + FB_NODE + '" 노드 .read/.write 권한 확인'
    });
    toast('서버 쓰기 실패 — F12 콘솔 확인','warn');
    return;
  }
  console.log('[方劑麻雀] AI 對局 방 생성 성공:', rid);

  // 짧은 대기 (Firebase 반영) 후 시작
  await new Promise(res => setTimeout(res, 200));
  const sr = await startGame(rid);
  if(sr && sr.ok === false){
    console.error('[方劑麻雀] startGame 실패:', sr.msg);
